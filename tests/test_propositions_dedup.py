...
def _seed(conn, uid, rows):
    ...
    for i, (text, vec) in enumerate(rows):
        # Derived facts require an existing message belonging to the same owner.
        conn.execute(
            "INSERT INTO messages (user_id,id,thread_id,from_addr,to_addr,date) "
            "VALUES (%s,%s,%s,'sender@example.test','owner@example.test','2020')",
            (uid, f"m{i}", f"t{i}"),
        )
        conn.execute(
...
def test_collapse_merges_near_dup_trio(db_backend, tmp_path):
    ...
    with get_connection(db_backend["db_path"]) as conn:

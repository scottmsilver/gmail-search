...
from firecracker_backend import SyntheticAgentMCPBackend,ROOT,JAILS,boundary
...
def main():
    ...
    assert len(raw)<=4096
    backend=SyntheticAgentMCPBackend()
    ...
    try:
        ...
        while time.monotonic()<deadline:
            ...
            if 'INNER_MAIL_MCP_PASS' in content.splitlines():break
            if 'INNER_MAIL_MCP_FAILED' in content.splitlines() or handle not in backend.inventory():
                raise RuntimeError('Synthetic guest workflow failed')
            time.sleep(.2)
        else:raise TimeoutError('Synthetic guest workflow deadline')
        print('\n'.join(line for line in content.splitlines() if line.startswith('INNER_MAIL_MCP')),flush=True)
    finally:

...
from firecracker_backend import SyntheticAgentPiMCPBackend,ROOT,JAILS,boundary
...
def main():
    ...
    assert len(raw)<=4096
    backend=SyntheticAgentPiMCPBackend()
    ...
    try:
        ...
        while time.monotonic()<deadline:
            ...
            if 'INNER_PI_MCP_PASS' in content.splitlines():break
            if 'INNER_PI_MCP_FAILED' in content.splitlines() or handle not in backend.inventory():
                print('\n'.join(line for line in content.splitlines() if line.startswith('INNER_PI_MCP')),flush=True)

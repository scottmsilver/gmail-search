...
-- Fresh installs use owner identities. Existing tables require the explicit
-- maintenance migration; startup never promotes legacy message keys.
CREATE TABLE IF NOT EXISTS messages (
    id TEXT NOT NULL,
    user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    -- `search_id` is the NUMERIC profile's message key. The live database has
    -- no such column and keys BM25 on `id`, so this line is installed only for
    -- the profiles that want it: `db._read_pg_schema()` removes the block below
    -- unless the selected schema profile is numeric. `migrate_owner_partitions.py`
    -- and the gateway search-reader fixtures select that profile explicitly.
    --
    -- Nothing here migrates an existing database. A database whose shape does
    -- not match the selected profile is refused at connect by
    -- `store/schema_profile.py`, never quietly upgraded.
...
CREATE TABLE IF NOT EXISTS attachments (
    ...
    UNIQUE (user_id, message_id, filename),
...
CREATE TABLE IF NOT EXISTS message_topics (
    ...
    PRIMARY KEY (user_id, message_id, topic_id)
);
...
-- its own embedding. Available through find_facts and read-only SQL;
-- gmail_search_reader reads are scoped to the active user by RLS below.
-- DDL lived in propositions.py (lazy
...
-- Existing indexes are never rebuilt by startup. Select the matching key only
-- when creating an absent index; legacy upgrades use migrate_owner_keys.py.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='messages'::regclass
               AND attname='search_id' AND NOT attisdropped) THEN
        CREATE INDEX IF NOT EXISTS messages_bm25_idx ON messages
            USING bm25 (search_id, id, subject, body_text, from_addr, to_addr)

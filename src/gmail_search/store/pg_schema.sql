...
CREATE TABLE IF NOT EXISTS messages (
    ...
    -- `search_id` makes a fresh install the NUMERIC profile, which
    -- `migrate_owner_partitions.py` and the gateway search-reader fixtures
    -- require. The live database has no such column and is keyed on `id`, so a
    -- fresh install and production are deliberately different shapes today.
    -- The TEXT installer that closes that gap is a separate file the caller
    -- inventory still owes; do not close it by editing this historical schema.

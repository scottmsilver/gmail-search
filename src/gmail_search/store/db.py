...
TABLE_DOCS: dict[str, str] = {
    ...
    "attachments": (
        ...
        "- embed_status (TEXT): NULL normally; 'failed_permanent' when the image could not be embedded (corrupt file,\n"
        "  rejected by the embedding API). Such attachments have no image embedding and never will.\n"
        "- embed_error (TEXT): human-readable reason for the embed failure, NULL if none.\n"
        "- embed_attempts (INT): number of embed passes that failed for this attachment.\n"
        "\n"
        "FAST FREE-TEXT SEARCH — use BM25, NOT `LIKE '%...%'`:\n"

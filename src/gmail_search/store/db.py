...
from gmail_search.store.schema_profile import selected_bm25_key, selected_profile
...
# `pg_schema.sql` carries DDL that only some schema profiles want, fenced like:
#
#     -- >>> profile-only: numeric-key-v1
#     search_id BIGINT GENERATED BY DEFAULT AS IDENTITY UNIQUE,
#     -- <<< profile-only: numeric-key-v1
#
# One file, one applier, and the installed shape follows the same trusted
# selection every reader is bound to — rather than a fresh install landing on a
# different shape from production and nothing noticing until a query fails.
_FENCE_MARKER = "profile-only:"
_PROFILE_FENCE = re.compile(
    r"^-- >>> profile-only: (?P<profiles>[\w., -]+)\n(?P<body>.*?)^-- <<< profile-only: (?P=profiles)\n",
    re.MULTILINE | re.DOTALL,
)


def _select_profile_fences(schema: str, profile_name: str) -> str:
    """Keep each fenced block only for the profiles that named it.

    A fence that fails to parse must not fall through. A missing closing marker,
    a closing marker whose profile list is not byte-identical to the opening
    one, or a CRLF checkout all make the pattern miss — and a miss leaves the
    fenced DDL in the text that gets installed, for every profile. That is the
    failure this whole mechanism exists to prevent, so an unmatched marker is an
    error rather than a silent pass-through.
    """

    def keep(match: re.Match) -> str:
        wanted = {name.strip() for name in match.group("profiles").split(",")}
        return match.group("body") if profile_name in wanted else ""

    selected = _PROFILE_FENCE.sub(keep, schema)
    if _FENCE_MARKER in selected:
        leftovers = [line.strip() for line in selected.splitlines() if _FENCE_MARKER in line]
        raise RuntimeError(
            "unbalanced profile fence in pg_schema.sql — these markers did not parse, so the "
            f"DDL they guard would install for every profile: {leftovers}")
    return selected


def _read_pg_schema() -> str:
    """The Postgres DDL for *this* process's schema profile.

    The single source for both installing the schema and introspecting it, so
    the table names the docs are checked against are the ones that get created.
    """
    return _select_profile_fences(_PG_SCHEMA_PATH.read_text(), selected_profile().name)
...
#
# These are templates, not finished text: `{bm25_key}` is filled in by
# `table_docs()` from the selected schema profile. Never write the key as a
# literal here — an example naming a column the database does not have is how
# the model gets handed SQL it cannot run.
# ─────────────────────────────────────────────────────────────────────
TABLE_DOCS: dict[str, str] = {
    "messages": (
        ...
        "  Syntax: `WHERE {bm25_key} @@@ '<tantivy-query>'`. Tantivy uses `field:term`,\n"
        "  combinable with AND / OR / NOT and parentheses. Order by\n"
        "  `paradedb.score({bm25_key}) DESC` for relevance ranking.\n"
        "  Examples:\n"
        "    -- Sender + subject keyword:\n"
        "    SELECT id, subject FROM messages\n"
        "    WHERE {bm25_key} @@@ 'from_addr:delta AND subject:cancel'\n"
        "    ORDER BY paradedb.score({bm25_key}) DESC LIMIT 50;\n"
        "    -- Multi-field OR (any of these fields contains 'invoice'):\n"
        "    SELECT id FROM messages\n"
        "    WHERE {bm25_key} @@@ 'subject:invoice OR body_text:invoice';\n"
        "    -- Exact phrase in body_text:\n"
        "    SELECT id FROM messages WHERE {bm25_key} @@@ 'body_text:\"refund issued\"';\n"
    ...
    "attachments": (
        ...
        "- embed_error (TEXT): human-readable reason for the last embed failure. With embed_status NULL it means a retry\n"
        "  is pending; NULL once every image is embedded (or if no image ever failed).\n"
...
_BM25_KEY_PLACEHOLDER = "{bm25_key}"


def table_docs() -> dict[str, str]:
    """`TABLE_DOCS` with the BM25 key resolved for this process's schema shape.

    Read this rather than `TABLE_DOCS` wherever the text is shown to anyone —
    the raw constant still carries the placeholder.
    """
    key = selected_bm25_key()
    return {name: doc.replace(_BM25_KEY_PLACEHOLDER, key) for name, doc in TABLE_DOCS.items()}
...
def _bind_schema_profile(wrapper) -> None:
    ...
    if _os.environ.get(_schema_profile.SKIP_CHECK_ENV):
        return

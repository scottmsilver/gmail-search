...
ATTACHMENT_BATCH_DESC = (
    ...
    "files <=1MB by default, PLUS a signed fetch_url you can GET directly "
    "with no auth for any size — `fetch_url_expires_at` (epoch seconds) "
    "and `fetch_url_ttl_sec` say how long it lives; inline:false for "
    "url-only). Rows with fetch_status != 'ok' have no bytes to fetch. `attachment_id` comes from a thread's "

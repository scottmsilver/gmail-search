...
from gmail_search.search.ranking import (  # noqa: F401 (compatibility re-exports)
    collapse_repeat_senders,
    filter_offtopic,
    normalize_subject,
    W_SIMILARITY,
    W_BM25,
    ...
    W_THREAD_SIZE,
    W_FRESH_MATCH,
    VECTOR_BRUTEFORCE_THRESHOLD,
    CANDIDATE_ID_CAP,
    RECENCY_HALF_LIFE_DAYS,
    LABEL_SCORES,
    _label_score,
    _recency_score,
    _match_density_score,
    _exact_subject_phrase,
    _thread_size_score,
    _contact_frequency_score,
    SearchResult,
    ThreadMatch,
    ThreadResult,
)
...
class SearchEngine:
        ...
        try:
            ...
            rows = conn.execute(
                f"""SELECT e.id, e.message_id, e.attachment_id, e.chunk_type, e.chunk_text,
                    ...
                    FROM embeddings e
                    JOIN messages m ON e.message_id = m.id AND e.user_id = m.user_id
                    LEFT JOIN attachments a ON e.attachment_id = a.id
                      AND a.user_id = e.user_id AND a.message_id = e.message_id
                    WHERE e.id IN ({placeholders}) AND e.user_id = %s""",
                [*embedding_ids, self.user_id],
            ).fetchall()
    ...
    _filter_offtopic = staticmethod(filter_offtopic)
    _normalize_subject = staticmethod(normalize_subject)
    _collapse_repeat_senders = staticmethod(collapse_repeat_senders)

"""Exercise fixed-reader policy after REINDEX and own/foreign native maintenance."""
import asyncio
import secrets
import os
...
async def test_three_owner_fixed_readers_survive_three_own_and_foreign_vacuum_cycles(request,fixture,profile,branch):
    ...
    try:
        ...
        # Synthetic alternative: recreate canonical attached index trees with
        # fresh OIDs, preserving every table/TOAST relation and data file.
        heaps = db.conn.execute("SELECT oid,relfilenode,reltoastrelid FROM pg_class WHERE relnamespace='gms_mail_partitions'::regnamespace AND relkind='r' ORDER BY oid").fetchall()
        old_indexes = db.conn.execute("SELECT c.oid FROM pg_class c JOIN pg_am a ON a.oid=c.relam WHERE a.amname='bm25' ORDER BY c.oid").fetchall()
        with db.conn.transaction():
            for table,(index,columns,key) in profile.indexes.items():
                db.conn.execute(sql.SQL('DROP INDEX {}').format(sql.Identifier('public',index)))
                db.conn.execute(sql.SQL('CREATE INDEX {} ON {} USING bm25({}) WITH(key_field={})').format(sql.Identifier(index),sql.Identifier('public',table),sql.SQL(',').join(map(sql.Identifier,columns)),sql.Literal(key)))
        assert db.conn.execute("SELECT oid,relfilenode,reltoastrelid FROM pg_class WHERE relnamespace='gms_mail_partitions'::regnamespace AND relkind='r' ORDER BY oid").fetchall() == heaps
        new_indexes = db.conn.execute("SELECT c.oid FROM pg_class c JOIN pg_am a ON a.oid=c.relam WHERE a.amname='bm25' ORDER BY c.oid").fetchall()
        assert not set(old_indexes)&set(new_indexes)

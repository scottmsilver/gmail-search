...
`runner_completed` is **untrusted display output**, not worker stop acknowledgement. The browser controller must obtain backend stop ACK, then persist the answer and mark its own final status. The serial marker `GMS_AGENT_RUNNER_COMPLETE` appears only after the owned bridge cleanup and capability-file removal; any top-level failure prints only `GMS_AGENT_RUNNER_FAILED` and exits 1. A revoked events capability may prevent any final guest event. Controllers therefore also monitor worker failure, cancellation and deadlines. The worker types the runner as `guest_agent.py || /sbin/reboot -f` (`firecracker_backend.guest_command`): a failed runner reboots its VM, which makes Firecracker exit, so the controller's next heartbeat (every 10 s) has its renewal refused and ends the run as failed instead of waiting out the 900 s deadline (#37). A completed runner leaves its VM for the controller to stop.

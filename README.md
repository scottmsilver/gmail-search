...
- **Jev (TypeSafe) in the invited service** — when `TYPESAFE_URL` and `TYPESAFE_API_KEY` are set (in `~/.config/gmail-search/invited-auth.env`), Jev does two jobs, and both are off without them. Before a Gemini run, `gateway/effort_router.py` asks Jev whether the question is a lookup, an investigation or an exhaustive sweep and binds thinking `LOW`, `MEDIUM` or `HIGH` (about 100–250 ms; `MEDIUM` on any error). During the run, the guest's `judge` tool (`POST /v1/judge`) lets the agent ask typed questions instead of guessing, and its instructions tell it to check a noul `answered` after each read. An `answered` below 0.5 raises thinking one level for the rest of the run (`gateway/escalation.py`, upgrade-only; the call-count backstop is off). The question text and evidence summaries the agent passes to `judge` are sent to TypeSafe.
- **Guest tool limits** — every mail tool call has a 5 s budget (`gateway/tool_deadline.py`, the guest's `TOOL_TIMEOUT_SECONDS`, and a 6 s adapter backstop). SQL that pattern-matches message or attachment text (`LIKE`/`ILIKE`/regex on `body_text`, `subject`, `extracted_text`, …) is rejected with a message pointing to `search_emails_batch`; those scans read the whole mailbox.
...
- **Invited gateway** — `gmail_search.invited_server` logs to the journal (`journalctl --user -u gmail-search-invited-api`): one `gateway <METHOD> <path> <status> <ms>` line per worker call (route and time only, never contents; 2 s or more logs as a warning), the Jev effort choice per run, thinking escalations, and the per-owner token budget (at startup, at 80% and 95% of the ceiling, and on refusal). A refused reservation returns HTTP 402 `Token budget exhausted: spent X of Y tokens`; the ceiling lives in the registry's `budgets` table.

...
- **Streaming** — `/api/deep` SSE proxies stream stages into the chat UI under a single AssistantWork disclosure. Each stage emits its cost, and per-turn cost + elapsed time are shown under the assistant response, surviving a page reload (reconstructed from `agent_events` the same way tool calls are); the `pi` backend also streams the model's interim prose between tool calls as `assistant` stages.

...
- **Multi-tenant** — with `GMAIL_MULTI_TENANT=1`, several people can use one deployment. Sign-in goes through the silver-oauth broker, only invited emails (`gmail-search invite`) can register, every row carries a `user_id`, and retrieval operations enforce the signed-in user. Arbitrary SQL is disabled; shared agent runtimes still require isolation before public deployment.

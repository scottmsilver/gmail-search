...
- **Guest tool limits** — every mail tool call has a 5 s budget (`gateway/tool_deadline.py`, the guest's `TOOL_TIMEOUT_SECONDS`, and a 6 s adapter backstop). SQL that pattern-matches message or attachment text (`LIKE`/`ILIKE`/regex on `body_text`, `extracted_text`, …; `subject` stays allowed) is rejected with a message pointing to `search_emails_batch`; those scans read the whole mailbox. Each `search_emails_batch` search also takes `sender` and `recipient` filters, which take precedence over `from:`/`to:` operators in the query text.

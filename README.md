...
- **Isolated runtimes (invited service)** — `gmail_search.invited_server` runs each deep turn in a Firecracker microVM on the dedicated worker. The browser's `backend` picks the guest: `claude_code` boots native Claude Code, `pi` boots Pi; `guest_agent.py` dispatches on the envelope's profile. The model is pinned by the gateway (`claude-sonnet-4-6`), and the guest holds only a per-run capability. Upstream Anthropic auth is a Claude login, used only for Claude Code runs: either `provider.claude_oauth_token` (from `claude setup-token`) or `provider.claude_login_file` (the host's `~/.claude/.credentials.json`, re-read per request and never refreshed or written, so the owner's own Claude Code keeps it fresh) and/or `provider.anthropic_key` (the only credential Pi runs may use). A runtime with no usable credential is refused before launch. The worker's wall clock is kept on the host's by `gmail-worker-clock-sync.service` (KVM PTP), because lease deadlines are compared across the two machines.
